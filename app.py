...
def render() -> None:
    ...
    with st.sidebar:
        ...
        search_catalog = [item for item in SEARCH_SUGGESTIONS if item["market"] == market]
        search_universe = US_UNIVERSE if market == "US" else INDIA_UNIVERSE
        for symbols in search_universe.values():
            for symbol in symbols:
                search_catalog.append({"market": market, "symbol": symbol if market == "US" else f"{symbol}.NS", "name": symbol, "exchange": "NASDAQ/NYSE" if market == "US" else "NSE"})
        label_to_match = {}
        for match in search_catalog:
            label = f"{match['name']} — {match['symbol']} ({match['exchange']})"
            label_to_match[label] = match
        selected_label = st.selectbox(
            "Type a company name or ticker",
            list(label_to_match),
            index=None,
            placeholder="Example: Aza, Apple, Infosys, Reliance",
            key=f"company-picker-{market}",
        )
        if selected_label:
            chosen_match = label_to_match[selected_label]
            if st.button("Load selected company", width="stretch"):
                st.session_state.active_ticker = display_symbol(chosen_match["symbol"])
                st.session_state.dual_listing = "Custom ticker"
                st.rerun()
